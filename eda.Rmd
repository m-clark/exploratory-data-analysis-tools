...
As far as reporting, SmartEDA also provides functionality.
...
Provides useful functionality you won't see in the other packages. 
...
The package visdat[^visdescr] is, as its name implies, purely for visualization, and this includes missingness, correlation, and more.  We can start by visualizing the missing data.
...
- Ready to use output: supplies a data.frame or ggplot object.
- Coding Quality: uses type-safe checks, active development (notable recent activity), explicitly uses some standard (e.g. open sci), my own personal opinion
...
- Dedicated Website: more than just a github repo, has a website with vignettes.  All of these are on github, so with pkgdown there is no reason not to have this.
...
[^nottoomessy]: However, for our demonstration I'll be using something a little more wieldy.
 ...
 [^visdescr]: The article by Staniak & Biecek places visdat with description rather than validity, but the data frame comparison, expectation investigation, and missing exploration seem more validity issues than data summaries.